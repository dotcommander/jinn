...
func TestClassifyCommand(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "timeout wrapper dangerous verb", cmdline: "timeout 30 rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "timeout option wrapper dangerous verb", cmdline: "timeout --signal KILL --kill-after=5s 30 rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "timeout wrapper safe verb", cmdline: "timeout 30 echo ok", wantLevel: RiskSafe},
		{name: "setsid wrapper dangerous verb", cmdline: "setsid rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "stdbuf wrapper dangerous verb", cmdline: "stdbuf -oL rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "time format wrapper dangerous verb", cmdline: "time --format '%e' rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "doas dangerous", cmdline: "doas rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "elevated privileges"},
		{name: "pkexec dangerous", cmdline: "pkexec rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "elevated privileges"},
		{name: "python explicit stdin program dangerous", cmdline: "python3 -", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "node explicit stdin program dangerous", cmdline: "node -", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "curl output to loop device dangerous", cmdline: "curl https://example.com/image.iso -o /dev/loop0", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl repeated output includes device dangerous", cmdline: "curl https://example.com/image.iso -o image.iso --output=/dev/loop0", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget repeated output includes device dangerous", cmdline: "wget https://example.com/image.iso -O image.iso --output-document=/dev/loop0", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "redirection to zvol dangerous", cmdline: "cat image.iso > /dev/zvol/tank/volume", wantLevel: RiskDangerous, wantSub: "redirection to device"},
		{name: "redirection to null stays caution", cmdline: "echo ok > /dev/null", wantLevel: RiskCaution, wantSub: "output redirection"},
		{name: "time ordinary output is caution", cmdline: "/usr/bin/time -o time.txt echo ok", wantLevel: RiskCaution, wantSub: "time output"},
		{name: "time device output is dangerous", cmdline: "/usr/bin/time --output=/dev/./sda echo ok", wantLevel: RiskDangerous, wantSub: "time output to device"},
		{name: "timeout wrapped time device output is dangerous", cmdline: "timeout 30 /usr/bin/time -o /dev/./sda echo ok", wantLevel: RiskDangerous, wantSub: "time output to device"},
		{name: "time no output preserves safe payload", cmdline: "/usr/bin/time echo ok", wantLevel: RiskSafe},
		{name: "time payload argument is not time output", cmdline: "/usr/bin/time echo -o /dev/sda", wantLevel: RiskSafe},
		{name: "time format then ordinary output is caution", cmdline: "time -f %e -o time.txt echo ok", wantLevel: RiskCaution, wantSub: "time output"},
		{name: "time format then device output is dangerous", cmdline: "time -f %e -o /dev/./sda echo ok", wantLevel: RiskDangerous, wantSub: "time output to device"},
		{name: "time equal format and output is caution", cmdline: "time --format=%e --output=time.txt echo ok", wantLevel: RiskCaution, wantSub: "time output"},
		{name: "time equal format and device output is dangerous", cmdline: "time --format=%e --output=/dev//sda echo ok", wantLevel: RiskDangerous, wantSub: "time output to device"},
		{name: "nested time ordinary output is caution", cmdline: "time time -o time.txt echo ok", wantLevel: RiskCaution, wantSub: "time output"},
		{name: "nested time device output is dangerous", cmdline: "time time -o /dev/../dev/sda echo ok", wantLevel: RiskDangerous, wantSub: "time output to device"},
		{name: "wrapper chain time ordinary output is caution", cmdline: "timeout 30 env time -o time.txt echo ok", wantLevel: RiskCaution, wantSub: "time output"},
		{name: "normalized redirection device is dangerous", cmdline: "cat image.iso > /dev//sda", wantLevel: RiskDangerous, wantSub: "redirection to device"},
		{name: "normalized curl device is dangerous", cmdline: "curl -o /dev/../dev/sda https://example.com/image.iso", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "normalized wget device is dangerous", cmdline: "wget --output-document=/dev/./sda https://example.com/image.iso", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "normalized tee device is dangerous", cmdline: "tee /dev/./sda", wantLevel: RiskDangerous, wantSub: "tee to device"},
		{name: "normalized copy device is dangerous", cmdline: "cp image.iso /dev/../dev/sda", wantLevel: RiskDangerous, wantSub: "to device"},
		{name: "normalized null remains non-dangerous", cmdline: "echo ok > /dev/./null", wantLevel: RiskCaution, wantSub: "output redirection"},
		{name: "mkfs family is dangerous", cmdline: "mkfs.btrfs /dev/sda", wantLevel: RiskDangerous, wantSub: "formats filesystem"},
		{name: "mkswap is dangerous", cmdline: "mkswap /dev/sda", wantLevel: RiskDangerous, wantSub: "swap area"},
		{name: "wipefs is dangerous", cmdline: "wipefs --all /dev/sda", wantLevel: RiskDangerous, wantSub: "filesystem signatures"},
		{name: "blkdiscard is dangerous", cmdline: "blkdiscard /dev/sda", wantLevel: RiskDangerous, wantSub: "block-device contents"},
		{name: "mkfs helper stays conservative caution", cmdline: "mkfs-helper --check", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "nested timeout env taskset dangerous", cmdline: "timeout 30 env -C . -a runner taskset -c 0 rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "long env options and taskset dangerous", cmdline: "timeout 30 env --chdir=. --argv0=runner taskset --cpu-list=0 rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "taskset safe payload stays safe", cmdline: "taskset -c 0 echo ok", wantLevel: RiskSafe},
		{name: "taskset attached cpu list dangerous payload", cmdline: "taskset -c0 rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "python option then stdin alias dangerous", cmdline: "python3 -W ignore /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "python option then stdin dash dangerous", cmdline: "python3 -X dev -", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "node option then stdin alias dangerous", cmdline: "node --require loader /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "perl option then stdin dash dangerous", cmdline: "perl -I lib -", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "python option script path stays caution", cmdline: "python3 -W ignore script.py", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "node option script path stays caution", cmdline: "node --require loader script.js", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "node conditions then stdin alias dangerous", cmdline: "node --conditions development /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "node conditions script path stays caution", cmdline: "node --conditions development script.js", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "ruby directory then stdin alias dangerous", cmdline: "ruby -C tmp /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "ruby long directory then stdin alias dangerous", cmdline: "ruby --directory=tmp /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "ruby directory script path stays caution", cmdline: "ruby -C tmp script.rb", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "php file stdin alias dangerous", cmdline: "php -f /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "php long file stdin alias dangerous", cmdline: "php --file=/dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "php file script path stays caution", cmdline: "php -f script.php", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "piped python stdin is dangerous", cmdline: "printf code | python3 -", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "curl trace device is dangerous", cmdline: "curl --trace /dev/./sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl trace ascii device is dangerous", cmdline: "curl --trace-ascii=/dev//sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl dump header device is dangerous", cmdline: "curl -D/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl repeated file writes include device", cmdline: "curl --trace trace.txt --dump-header=/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl non-writing trace time stays caution", cmdline: "curl --trace-time https://example.com", wantLevel: RiskCaution},
		{name: "wget output log device is dangerous", cmdline: "wget -o/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget append output device is dangerous", cmdline: "wget --append-output=/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget hsts device is dangerous", cmdline: "wget --hsts-file=/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget ordinary log output stays caution", cmdline: "wget --output-file=wget.log https://example.com", wantLevel: RiskCaution},
		{name: "curl etag save device is dangerous", cmdline: "curl --etag-save /dev/./sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl libcurl device is dangerous", cmdline: "curl --libcurl=/dev//sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl etag and libcurl repeated device is dangerous", cmdline: "curl --etag-save etag.txt --libcurl=/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "curl etag ordinary file stays caution", cmdline: "curl --etag-save etag.txt https://example.com", wantLevel: RiskCaution},
		{name: "curl libcurl ordinary file stays caution", cmdline: "curl --libcurl client.c https://example.com", wantLevel: RiskCaution},
		{name: "wget save cookies device is dangerous", cmdline: "wget --save-cookies /dev/./sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget rejected log device is dangerous", cmdline: "wget --rejected-log=/dev//sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget cookie and rejected log repeated device is dangerous", cmdline: "wget --save-cookies cookies.txt --rejected-log=/dev/sda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "wget save cookies ordinary file stays caution", cmdline: "wget --save-cookies cookies.txt https://example.com", wantLevel: RiskCaution},
		{name: "wget rejected log ordinary file stays caution", cmdline: "wget --rejected-log rejected.txt https://example.com", wantLevel: RiskCaution},
		{name: "env BSD utilpath wrapper dangerous", cmdline: "env -P /usr/bin rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "env attached BSD utilpath wrapper dangerous", cmdline: "env -P/usr/bin rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "nested env BSD utilpath wrapper dangerous", cmdline: "timeout 30 env -P /usr/bin taskset -c 0 rm -rf /tmp/x", wantLevel: RiskDangerous, wantSub: "force/recursive"},
		{name: "curl write out device separate dangerous", cmdline: "curl -w '%output{/dev/./sda}' https://example.com", wantLevel: RiskDangerous, wantSub: "write-out to device"},
		{name: "curl write out device attached dangerous", cmdline: "curl -w'%output{>>/dev/sda}' https://example.com", wantLevel: RiskDangerous, wantSub: "write-out to device"},
		{name: "curl write out device equal dangerous", cmdline: "curl --write-out=%output{/dev//sda} https://example.com", wantLevel: RiskDangerous, wantSub: "write-out to device"},
		{name: "curl write out repeated includes device dangerous", cmdline: "curl -w '%output{metrics.txt}%output{>>/dev/sda}' https://example.com", wantLevel: RiskDangerous, wantSub: "write-out to device"},
		{name: "curl write out ordinary destination stays caution", cmdline: "curl --write-out='%output{metrics.txt}' https://example.com", wantLevel: RiskCaution},
		{name: "PHP begin code dangerous", cmdline: "php -B 'echo 1;'", wantLevel: RiskDangerous, wantSub: "inline code"},
		{name: "PHP attached record code dangerous", cmdline: "php -R'echo $_;'", wantLevel: RiskDangerous, wantSub: "inline code"},
		{name: "PHP end code dangerous", cmdline: "php -E 'echo 1;'", wantLevel: RiskDangerous, wantSub: "inline code"},
		{name: "node conditions alias then stdin dangerous", cmdline: "node -C development /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "node experimental loader then stdin dangerous", cmdline: "node --experimental-loader loader /dev/stdin", wantLevel: RiskDangerous, wantSub: "explicit stdin"},
		{name: "node input type implicit stdin dangerous", cmdline: "node --input-type=module", wantLevel: RiskDangerous, wantSub: "input type without script"},
		{name: "node input type script stays caution", cmdline: "node --input-type=module script.js", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "rbd device destination dangerous", cmdline: "cp image.iso /dev/rbd0", wantLevel: RiskDangerous, wantSub: "to device"},
		{name: "drbd device redirection dangerous", cmdline: "cat image.iso > /dev/drbd0", wantLevel: RiskDangerous, wantSub: "redirection to device"},
		{name: "pmem device curl dangerous", cmdline: "curl -o /dev/pmem0 https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "bcache device tee dangerous", cmdline: "tee /dev/bcache0", wantLevel: RiskDangerous, wantSub: "tee to device"},
		{name: "dasd device wget dangerous", cmdline: "wget -O /dev/dasda https://example.com", wantLevel: RiskDangerous, wantSub: "output to device"},
		{name: "python module args do not imply stdin dangerous", cmdline: "python3 -m tool /dev/stdin", wantLevel: RiskCaution, wantSub: "unknown command"},
		{name: "curl remote output directory device dangerous", cmdline: "curl --output-dir /dev -O https://example.com/image.iso", wantLevel: RiskDangerous, wantSub: "remote output directory"},
		{name: "curl remote name all directory device dangerous", cmdline: "curl --remote-name-all --output-dir=/dev/./disks https://example.com/image.iso", wantLevel: RiskDangerous, wantSub: "remote output directory"},
		{name: "curl ordinary output directory stays caution", cmdline: "curl --output-dir downloads -O https://example.com/image.iso", wantLevel: RiskCaution},
		{name: "wget directory prefix device dangerous", cmdline: "wget -P /dev https://example.com/image.iso", wantLevel: RiskDangerous, wantSub: "output directory"},
		{name: "wget directory prefix equal device dangerous", cmdline: "wget --directory-prefix=/dev/./disks https://example.com/image.iso", wantLevel: RiskDangerous, wantSub: "output directory"},
		{name: "wget ordinary directory prefix stays caution", cmdline: "wget -P downloads https://example.com/image.iso", wantLevel: RiskCaution},
