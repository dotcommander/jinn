...
func TestClassifyCommand(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "pnpm install caution", cmdline: "pnpm install", wantLevel: RiskCaution},
		{name: "bun install caution", cmdline: "bun install", wantLevel: RiskCaution},
		...
		{name: "awk program dangerous", cmdline: "awk '{ system($0) }' commands.txt", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "gawk program dangerous", cmdline: "gawk -f script.awk input.txt", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "make target dangerous", cmdline: "make build", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "npx package dangerous", cmdline: "npx cowsay hello", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "pnpm dlx dangerous", cmdline: "pnpm dlx cowsay hello", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "pnpm exec dangerous", cmdline: "pnpm exec eslint .", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "bunx package dangerous", cmdline: "bunx eslint .", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "bun x dangerous", cmdline: "bun x eslint .", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "env wrapper awk dangerous", cmdline: "env LC_ALL=C awk '{ print $1 }' input.txt", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "command wrapper awk dangerous", cmdline: "command awk '{ print $1 }' input.txt", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "time wrapper awk dangerous", cmdline: "time -p awk '{ print $1 }' input.txt", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "pnpm global dir then dlx dangerous", cmdline: "pnpm --dir . dlx cowsay hello", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		{name: "bun global cwd then x dangerous", cmdline: "bun --cwd . x eslint .", wantLevel: RiskDangerous, wantSub: "arbitrary code execution"},
		...
		{name: "nested arithmetic comparisons no redirection", cmdline: "(( (a > b) && ((c > d)) ))", wantLevel: RiskSafe},
		{name: "double-bracket test comparison no redirection", cmdline: "[[ $a > $b ]]", wantLevel: RiskSafe},
		{name: "compound double-bracket comparison no redirection", cmdline: "[[ $a == x && $b > y ]]", wantLevel: RiskSafe},
		{name: "unicode double-bracket comparison no redirection", cmdline: "if [[ café > thé ]]; then echo ok; fi", wantLevel: RiskSafe},
		{name: "arithmetic expansion in argument no redirection", cmdline: "echo $((a > b))", wantLevel: RiskSafe},
		{name: "quoted redirection stays safe", cmdline: `echo "> out.txt"`, wantLevel: RiskSafe},
		{name: "escaped redirection stays safe", cmdline: `echo \> out.txt`, wantLevel: RiskSafe},
		...
		{name: "nested arithmetic comparison with redirection caution", cmdline: "(( (a > b) && ((c > d)) )) > out.txt", wantLevel: RiskCaution, wantSub: "redirection"},
		{name: "double-bracket comparison with redirection caution", cmdline: "[[ $a > $b ]] > out.txt", wantLevel: RiskCaution, wantSub: "redirection"},
		{name: "unicode comparison with later redirection caution", cmdline: "if [[ café > thé ]]; then echo ok > out.txt; fi", wantLevel: RiskCaution, wantSub: "redirection"},
