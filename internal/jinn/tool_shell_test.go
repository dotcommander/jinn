...
func TestRunShell_Echo(t *testing.T) {
	t.Parallel()
	e, _ := testEngine(t)
	result := e.runShell(context.Background(), args("command", "echo hello"))
...
func TestRunShell_ExitCode(t *testing.T) {
	t.Parallel()
	e, _ := testEngine(t)
	result := e.runShell(context.Background(), args("command", "exit 42"))
...
func TestRunShell_TimeoutClamp(t *testing.T) {
	t.Parallel()
	e, _ := testEngine(t)
	result := e.runShell(context.Background(), args("command", "echo ok", "timeout", float64(9999)))
...
func TestRunShell_DryRun(t *testing.T) {
	t.Parallel()
	e, _ := testEngine(t)
	result := e.runShell(context.Background(), args("command", "rm -rf /", "dry_run", true))
...
func TestRunShell_CollapseRepeated(t *testing.T) {
	t.Parallel()
	e, _ := testEngine(t)
	result := e.runShell(context.Background(), args("command", "for i in $(seq 1 10); do echo same; done"))
