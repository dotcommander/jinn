...
func condenseGitLog(output string) string {
	...
	// Walk lines, collecting commit blocks and emitting one line per commit.
	var emitted []string
	var hash, decoration, subject string
	isMerge := false
	inBlock := false
	...
	flush := func() {
		...
		emitted = append(emitted, sb.String())
	}
	...
	for _, line := range lines {
		...
		if len(line) >= 4 && line[:4] == "    " {
			...
			// Body continuation lines are skipped.
			continue
		}
