...
func (p *renderProxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	p.serveForward(w, req)
}

func (p *renderProxy) serveForward(w http.ResponseWriter, req *http.Request) {
	...
	if err := validateURL(req.URL, p.client.allowPrivateNetworks); err != nil {
		p.reject(w, err)
		return
	}
...
func (p *renderProxy) serveConnect(w http.ResponseWriter, req *http.Request) {
	...
	target := &url.URL{Scheme: schemeHTTPS, Host: authority}
	if err := validateURL(target, p.client.allowPrivateNetworks); err != nil {
		p.reject(w, err)
		return
	}
