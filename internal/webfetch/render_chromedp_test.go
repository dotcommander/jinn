...
func TestRendererChromeFlagsForceProxyEgress(t *testing.T) {
	t.Parallel()
	flags := rendererChromeFlags("http://127.0.0.1:1234", "jinn/test-agent")
...
func TestValidateRenderRequestURLRejectsNonHTTPAndUnsafeTargets(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			assertCodedError(t, validateRenderRequestURL(tt.raw, false), tt.code)
		})
	}
}
...
func TestChromedpRendererControlledPage(t *testing.T) {
	chromePath := strings.TrimSpace(os.Getenv("JINN_WEB_TEST_CHROME_PATH"))
	if chromePath == "" {
		t.Skip("JINN_WEB_TEST_CHROME_PATH is not set")
	}
