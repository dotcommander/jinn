...
func (p readerProvider) fetchDefuddleHTML(ctx context.Context, target *url.URL) (defuddleHTML, error) {
	...
	if !isHTMLContentType(contentType) {
		contentTypeErr := fmt.Errorf("defuddle fetch content type %q: %w", contentType, ErrNotHTML)
		return defuddleHTML{}, newCodedError(contentTypeErr, ErrorCodeNotHTML, "use -raw for non-HTML content or provide an HTML page")
	}
...
func decodeHTML(body []byte, contentType string) (string, error) {
	...
	if err != nil {
		// The original bytes remain a useful, bounded fallback when charset
		// metadata is malformed or unsupported.
		//nolint:nilerr // Returning raw content is the intentional fallback contract.
