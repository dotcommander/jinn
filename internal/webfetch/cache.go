...
func defaultURLCacheDir() (string, error) {
	if cacheHome := strings.TrimSpace(os.Getenv("XDG_CACHE_HOME")); cacheHome != "" {
		return filepath.Join(cacheHome, "jinn", "web", "urls"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".cache", "jinn", "web", "urls"), nil
}

//nolint:gocyclo // Cache validation is intentionally fail-closed and field-by-field.
...
func (c *urlCache) store(rawURL, readerMode, endpoint string, doc Document, renderArgs ...string) {
	...
	if mkdirErr := os.MkdirAll(c.dir, urlCacheDirectoryMode); mkdirErr != nil {
		return
	}
	if chmodErr := os.Chmod(c.dir, urlCacheDirectoryMode); chmodErr != nil { //nolint:gosec // enforce private cache directory.
		return
	}

	tmp, err := os.CreateTemp(c.dir, ".jinn-web-cache-*")
	if err != nil {
		return
	}
	tmpPath := tmp.Name()
...
func cacheRenderIdentity(renderArgs []string) (renderMode, renderWait string) {
