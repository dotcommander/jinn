...
//nolint:revive // The table intentionally covers every rejected resolver outcome in one contract.
func TestClientDialRejectsUnsafeResolution(t *testing.T) {
	t.Parallel()
	publicIP := net.ParseIP("93.184.216.34")
	privateIP := net.ParseIP("127.0.0.1")
	sharedIP := net.ParseIP("100.64.0.1")
	tests := []struct {
		name     string
		ips      []net.IPAddr
		err      error
		...
		{name: "shared address result", ips: []net.IPAddr{{IP: sharedIP}}, wantCode: ErrorCodePrivateNetwork},
...
func TestIsPrivateIPRejectsSpecialUseRanges(t *testing.T) {
	...
	tests := []struct {
		...
		{address: "2002:0a00:0001::", blocked: true},
		{address: "::ffff:100.64.0.1", blocked: true},
	}
	for _, test := range tests {
		t.Run(test.address, func(t *testing.T) {
			t.Parallel()
			if got := isPrivateIP(net.ParseIP(test.address)); got != test.blocked {
				t.Fatalf("isPrivateIP(%q) = %t, want %t", test.address, got, test.blocked)
			}
		})
	}
}
