...
type Service struct {
	...
	renderMu      sync.Mutex
	renderFactory func(rendererConfig) (pageRenderer, error)
	renderInit    chan struct{}
...
//
//nolint:funlen,gocyclo // Default normalization stays centralized so every provider receives one coherent policy.
func NewService(cfg Config) *Service {
	...
	if searchProviderName != "" && searchProviderName != providerBrave && searchProviderName != providerExa {
	...
	return &Service{
		...
		renderFactory: newChromedpRenderer,
		renderConfig: rendererConfig{
...
func (s *Service) Fetch(ctx context.Context, req FetchRequest) (Document, error) {
	...
	target, err := validateTarget(req.URL, s.client.allowPrivateNetworks)
	if err != nil {
		return Document{}, err
	}
	if req.Raw {
		if _, renderOptionErr := normalizeRenderOptions(req, ReaderModeDefuddle); renderOptionErr != nil {
			return Document{}, renderOptionErr
		}
...
//nolint:nestif // Auto rendering keeps its static, rendered, and safe-fallback order explicit.
...
//nolint:gocognit,gocyclo,nestif,revive // The fallback ladder is ordered policy and must remain visible in one place.
func (s *Service) fetchRenderedAuto(ctx context.Context, target *url.URL, options renderOptions) (Document, error) {
	if options.Mode == RenderModeAlways {
		...
		return Document{}, newCodedError(
			fmt.Errorf("auto reader failed: rendered defuddle: %w; jina: %w", renderErr, jinaErr),
	...
	doc, jinaErr := s.reader.fetch(ctx, target)
	if jinaErr == nil {
		return doc, nil
	}
	if staticErr != nil {
		return Document{}, newCodedError(
			fmt.Errorf("auto reader failed: defuddle: %w; jina: %w", staticErr, jinaErr),
			ErrorCodeReaderFallback,
			"use reader jina, reader defuddle, or render always to choose one path",
		)
	}
	return Document{}, newCodedError(
		fmt.Errorf("auto reader failed: defuddle extraction failed; jina: %w", jinaErr),
		ErrorCodeReaderFallback,
		"use reader jina, reader defuddle, or render always to choose one path",
	)
}
...
func (s *Service) render(ctx context.Context, target *url.URL, options renderOptions) (renderedPage, error) {
	renderer, err := s.ensureRenderer(ctx)
	if err != nil {
...
//nolint:gocognit,revive // This single-flight lifecycle keeps renderer ownership and close races explicit.
func (s *Service) ensureRenderer(ctx context.Context) (pageRenderer, error) {
	for {
		s.renderMu.Lock()
		if s.closed {
			s.renderMu.Unlock()
		...
		if s.renderInit != nil {
			initialized := s.renderInit
			s.renderMu.Unlock()
			select {
			case <-initialized:
				continue
			case <-ctx.Done():
				return nil, codeContextError(ctx.Err())
			}
		}

		initialized := make(chan struct{})
		s.renderInit = initialized
		factory := s.renderFactory
		config := s.renderConfig
		s.renderMu.Unlock()

		renderer, err := factory(config)
		s.renderMu.Lock()
		if s.closed {
			s.renderInit = nil
			s.renderMu.Unlock()
			if renderer != nil {
				_ = renderer.Close(ctx)
			}
			close(initialized)
			return nil, serviceClosedError()
		}
		if err != nil {
			...
			if errors.As(err, &coded) {
				...
				s.renderErr = newCodedError(fmt.Errorf("initialize browser renderer: %w", err), ErrorCodeBrowserUnavailable, "install Chrome or Chromium and configure its executable path")
			}
		} else {
			s.renderer = renderer
		}
...
func serviceClosedError() error {
	return newCodedError(errors.New("webfetch service is closed"), ErrorCodeInvalidArgument, "create a new webfetch service before rendering another page")
}
