...
func (p exaProvider) search(ctx context.Context, req SearchRequest) ([]SearchResult, error) {
	if strings.TrimSpace(p.apiKey) == "" {
		return nil, newCodedError(errors.New("EXA_API_KEY is required for Exa search"), ErrorCodeMissingAPIKey, "set EXA_API_KEY before selecting the exa search provider")
	}
