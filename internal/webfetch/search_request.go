...
func ParseSearchRequest(args map[string]any) (SearchRequest, error) {
	...
	if !ok || strings.TrimSpace(query) == "" {
		return SearchRequest{}, newCodedError(errors.New("web_search: 'query' is required"), ErrorCodeInvalidArgument, `valid shape: {"query":"search terms"}`)
	}
...
func normalizeSearchDate(value string) (string, error) {
	...
	return "", newCodedError(errors.New("start_published_date must be RFC3339 or YYYY-MM-DD"), ErrorCodeInvalidArgument, "use a timestamp such as 2026-08-01T00:00:00Z")
}
